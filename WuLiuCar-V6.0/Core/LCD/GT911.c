...
{
	return HAL_I2C_Mem_Write(&hi2c3, GT911_DIV_W, _usRegAddr, I2C_MEMADD_SIZE_16BIT, _pRegBuf, _ucLen, 100);
}

uint8_t GT911_ReadReg(uint16_t _usRegAddr, uint8_t *_pRegBuf, uint8_t _ucLen)
{
	return HAL_I2C_Mem_Read(&hi2c3, GT911_DIV_R, _usRegAddr, I2C_MEMADD_SIZE_16BIT, _pRegBuf, _ucLen, 100);
}
...
void TOUCH_INIT()
{
	...
	Address_SET_0xBA();
	dddddd=Software_Reset(1);
	HAL_Delay(100);
	dddddd=Software_Reset(0);
}