...
HAL_StatusTypeDef W25Q256_Read_ID(OSPI_HandleTypeDef* hospi, uint8_t* register_data)
{
    ...
    /* Instruction */
    sCommand.Instruction 			= W25Q_READ_JEDEC_ID_CMD;										/* What We Do? */
    /* Address */
    ...
    sCommand.NbData            		= 3;										/* Bytes Send With Data */
...
HAL_StatusTypeDef W25Q256_OCTO_SPI_Init(OSPI_HandleTypeDef* hospi)
{
	...
	if (W25Q256_OSPI_ResetChip(hospi) != HAL_OK) {
	    return HAL_ERROR;
	}
	if (W25Q256_OSPI_Configuration(hospi) != HAL_OK) {
        return HAL_ERROR;
    }
	HAL_Delay(1);
	if (W25Q256_OSPI_AutoPollingMemReady(hospi) != HAL_OK) {
        return HAL_ERROR;
    }
    if (W25Q256_OSPI_WriteEnable(hospi) != HAL_OK) {
        return HAL_ERROR;
    }
    return HAL_OK;
}
...
HAL_StatusTypeDef W25Q256_OSPI_Configuration(OSPI_HandleTypeDef* hospi)
{
    ...
    if (W25Q256_Read_Status_Registers(hospi, &reg1, 1) != HAL_OK)
    {
        return HAL_ERROR;
    }

    if (W25Q256_Read_Status_Registers(hospi, &reg2, 2) != HAL_OK)
    {
        return HAL_ERROR;
    }

    if (W25Q256_Read_Status_Registers(hospi, &reg3, 3) != HAL_OK)
    {
        return HAL_ERROR;
    }
    ...
    w_reg3 = (reg3 & W25Q_SR_DRV1) | W25Q_SR_ADP;

    if (W25Q256_Write_Status_Registers(hospi, w_reg1, 1) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (W25Q256_Write_Status_Registers(hospi, w_reg2, 2) != HAL_OK)
    {
        return HAL_ERROR;
    }

    if (W25Q256_Write_Status_Registers(hospi, w_reg3, 3) != HAL_OK)
    {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef W25Q256_OSPI_Erase_Chip(OSPI_HandleTypeDef* hospi)
{
    ...
    while (W25Q256_IsBusy(hospi)==HAL_ERROR)
    {
    	HAL_Delay(1);
    }
...
HAL_StatusTypeDef W25Q256_OSPI_EraseSector(OSPI_HandleTypeDef* hospi, uint32_t EraseStartAddress, uint32_t EraseEndAddress)
{
    ...
    while (EraseEndAddress >= StartAddress)
    {
    	...
    	sCommand.Address					= StartAddress;				                /* Byte Address */
    	/* Data */
    	sCommand.DataMode          			= HAL_OSPI_DATA_NONE;						/* Define Data Lines: No Data */
    	sCommand.DummyCycles       			= 0;										/* Bytes Send With No Data */
    	sCommand.NbData            			= 0;										/* Bytes Send With Data */
...
HAL_StatusTypeDef W25Q256_IsBusy(OSPI_HandleTypeDef* hospi)
{
	...
	state = W25Q256_Read_Status_Registers(hospi, &status_rgister, 1);
	if (state != HAL_OK)
		return state;

	status_rgister = status_rgister & 0xb1;
