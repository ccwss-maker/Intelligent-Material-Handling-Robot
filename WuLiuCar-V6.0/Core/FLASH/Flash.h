...
/**
  *  W25Q256 Configuration
  */
#define W25Q_FLASH_SIZE                  			0x10000000 /* 256 MBits => 16MBytes  256x64x1024*/
...
/**
 *  W25Q256 Commands W25Q Chip's Commands
 */
...
#define W25Q_READ_DATA_CMD 							0x13U	/* read data by standard SPI */
#define W25Q_FAST_READ_CMD 							0x0CU	/* highest FR speed (8.2.12) */

#define W25Q_PAGE_PROGRAM_CMD 						0x12U	/* program page (256bytes) by single SPI line */

#define W25Q_SECTOR_ERASE_CMD 						0x21U	/* sets all 4Kbyte sector with 0xFF (erases it) */
#define W25Q_64KB_BLOCK_ERASE_CMD 					0xDCU	/* sets all 64Kbyte block with 0xFF */
...
#define W25Q_FAST_READ_DUAL_OUT_CMD 				0x3CU	/* fast read in dual-SPI OUTPUT (8.2.14) */
#define W25Q_FAST_READ_DUAL_IO_CMD 					0xBCU	/* fast read in dual-SPI I/O (address transmits by both lines) */
#define W25Q_FULLID_DUAL_IO_CMD 					0x92U	/* read Manufacturer ID & Device ID by dual I/O */
#define W25Q_PAGE_PROGRAM_QUAD_INP_CMD 				0x34U	/* program page (256bytes) by Quad SPI lines */
#define W25Q_FAST_READ_QUAD_OUT_CMD 				0x6CU	/* fast read in quad-SPI OUTPUT (8.2.16) */
#define W25Q_FULLID_QUAD_IO_CMD 					0x94U	/* read Manufacturer ID & Device ID by quad I/O */
#define W25Q_FAST_READ_QUAD_IO_CMD 					0xECU	/* fast read in quad-SPI I/O (address transmits by quad lines) */
...
#define W25Q_SR_ADP                     			((uint8_t)0x02)    /*!< 4-Byte Address Enable*/

HAL_StatusTypeDef W25Q256_OCTO_SPI_Init(OSPI_HandleTypeDef* hospi);
HAL_StatusTypeDef W25Q256_OSPI_ResetChip(OSPI_HandleTypeDef* hospi);
HAL_StatusTypeDef W25Q256_OSPI_Configuration(OSPI_HandleTypeDef* hospi);

HAL_StatusTypeDef W25Q256_OSPI_Write_Command(OSPI_HandleTypeDef* hospi, uint8_t Command);

HAL_StatusTypeDef W25Q256_OSPI_WriteEnable(OSPI_HandleTypeDef* hospi);
HAL_StatusTypeDef W25Q256_OSPI_AutoPollingMemReady(OSPI_HandleTypeDef* hospi);
HAL_StatusTypeDef W25Q256_OSPI_Erase_Chip(OSPI_HandleTypeDef* hospi);
HAL_StatusTypeDef W25Q256_OSPI_EraseSector(OSPI_HandleTypeDef* hospi, uint32_t EraseStartAddress, uint32_t EraseEndAddress);
HAL_StatusTypeDef W25Q256_IsBusy(OSPI_HandleTypeDef* hospi);


HAL_StatusTypeDef W25Q256_OSPI_Write(OSPI_HandleTypeDef* hospi, uint8_t* pData, uint32_t WriteAddr, uint32_t Size);
HAL_StatusTypeDef W25Q256_OSPI_Read(OSPI_HandleTypeDef* hospi,uint8_t* pData, uint32_t ReadAddr, uint32_t Size);
HAL_StatusTypeDef W25Q256_OSPI_EnableMemoryMappedMode(OSPI_HandleTypeDef* hospi);

HAL_StatusTypeDef W25Q256_Read_ID(OSPI_HandleTypeDef* hospi, uint8_t* register_data);
HAL_StatusTypeDef W25Q256_Read_Status_Registers(OSPI_HandleTypeDef* hospi, uint8_t* register_data, uint8_t register_num);
HAL_StatusTypeDef W25Q256_Write_Status_Registers(OSPI_HandleTypeDef* hospi, uint8_t reg_data, uint8_t reg_num);
