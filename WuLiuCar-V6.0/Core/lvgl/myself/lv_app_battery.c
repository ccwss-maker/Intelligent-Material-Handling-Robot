...
void Battery_create(lv_obj_t * parent)
{
	...
	/*电压*/
	cont = create_bar(section, NULL, NULL, "油量", &my_style_font_DingTalk_JinBuTi_Chinese_16, 0, 100, 0);
	lv_obj_Battery.label_voltage = lv_obj_get_child(cont, 1);
	lv_label_set_text(lv_obj_Battery.label_voltage, "3.9L");
	lv_obj_Battery.label_percent = lv_obj_get_child(cont, 2);
	lv_label_set_text(lv_obj_Battery.label_percent, "66%");
	lv_obj_Battery.bar_percent = lv_obj_get_child(cont, 3);
	lv_bar_set_value(lv_obj_Battery.bar_percent, 66, LV_ANIM_ON);
}
