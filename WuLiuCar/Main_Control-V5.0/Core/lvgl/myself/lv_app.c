...
{	
    ...
    /*隐藏界面*/
    /*搬运*/
    lv_obj_t * sub_page_Transport = lv_menu_page_create(menu, NULL);
    lv_obj_set_style_pad_hor(sub_page_Transport, lv_obj_get_style_pad_left(lv_menu_get_main_header(menu), 0), 0);
	Transport_create(sub_page_Transport);
    /*PID*/
    ...
    /*搬运*/
    cont = create_text(section, USR_SYMBOL_Transport, &my_style_pic, "搬运", &my_style_font_DingTalk_JinBuTi_Chinese_16);
    lv_menu_set_load_page_event(menu, cont, sub_page_Transport);
    lv_obj_add_event_cb(cont, Page_Btn_Event_CB,LV_EVENT_ALL,(void*)0);
    /*运动*/
    ...
    lv_obj_add_event_cb(cont, Page_Btn_Event_CB,LV_EVENT_ALL,(void*)7);
    /*TF卡*/
...
void LVGL_Display()
{    
    if(Page_Sign.Transport)                 Camera_Msg_Display();
    else if(Page_Sign.Sport)                PID_Display();
