...
static lv_obj_t * Real_Position_Create(lv_obj_t *parent, const char *title)
{
    lv_obj_t * obj = lv_menu_cont_create(parent);
    static lv_coord_t col_dsc[] = {0, 3, 43, 58, 23, 24, LV_GRID_TEMPLATE_LAST};
	static lv_coord_t row_dsc[] = {15, 15, 12, LV_GRID_TEMPLATE_LAST}; /*3 100 px tall rows*/
    lv_obj_set_grid_dsc_array(obj, col_dsc, row_dsc);
    lv_obj_t * label = NULL;
    ...
    lv_obj_set_grid_cell(label, LV_GRID_ALIGN_CENTER, 3, 1, LV_GRID_ALIGN_START, 1, 1);
    ...
    lv_obj_add_style(label,&my_style_font_DingTalk_JinBuTi_English_14,LV_STATE_DEFAULT);
    lv_label_set_text(label, "0");
    lv_obj_set_grid_cell(label, LV_GRID_ALIGN_CENTER, 3, 1, LV_GRID_ALIGN_START, 2, 1);

    label = lv_label_create(obj);
    lv_obj_add_style(label,&my_style_font_DingTalk_JinBuTi_English_14,LV_STATE_DEFAULT);
    lv_label_set_text(label, "0");
    lv_obj_set_grid_cell(label, LV_GRID_ALIGN_START, 5, 1, LV_GRID_ALIGN_START, 2, 1);
...
void lv_Arm_Coord_Control()
{
    ...
    if(Arm_Coord_Control(&Point, &Angle, Arm_Velocity_Default) == true)
    {
        char buf[20];
...
static lv_obj_t * lv_menu_page_create_end_control(lv_obj_t *parent)
{
    ...
    /*X滑杆*/
    cont = create_slider_two(section, NULL, &my_style_pic, "X", &my_style_font_DingTalk_JinBuTi_English_14, Arm.Range.X_Min, Arm.Range.X_Max, Arm_Coord_X_0);
    ...
    /*Y滑杆*/
    cont = create_slider_two(section, NULL, &my_style_pic, "Y", &my_style_font_DingTalk_JinBuTi_English_14, Arm.Range.Y_Min, Arm.Range.Y_Max, Arm_Coord_Y_0);
    ...
    /*Z滑杆*/
    cont = create_slider_two(section, NULL, &my_style_pic, "Z", &my_style_font_DingTalk_JinBuTi_English_14, Arm.Range.Z_Min, Arm.Range.Z_Max, Arm_Coord_Z_0);
...
static void btn_rst_event_handler(lv_event_t * e)
{
    ...
    {
        Arm_slider_clear();
        Motor_Absolute_Control(Arm_Rear_Address, Arm_Velocity_Default, Arm_Accelerate, Rear_Angle_To_0_Set);
        Motor_Absolute_Control(Arm_Front_Address, Arm_Velocity_Default, Arm_Accelerate, Front_Angle_To_0_Set);
        Motor_Absolute_Control(Arm_Pedestal_Address, Arm_Velocity_Default, Arm_Accelerate, Pedestal_Angle_To_0_Set);
    }
}
...
static void slider_event_cb_Arm_Debug(lv_event_t * e)
{
    ...
    lv_snprintf(buf, sizeof(buf), "%d", slider_value);
    lv_label_set_text(label, buf);
    Motor_Absolute_Control(address, Arm_Velocity_Default, Arm_Accelerate, (float)slider_value);
}
...
static lv_obj_t * lv_menu_page_create_motor_debug(lv_obj_t *parent)
{
    ...
    /*大臂滑杆*/
    cont = create_slider_two(section, NULL, &my_style_pic, "大臂", &my_style_font_DingTalk_JinBuTi_Chinese_16, (int)Arm_Angle_a_Min, (int)Arm_Angle_a_Max, Rear_Angle_To_0_Set);
    ...
    /*小臂滑杆*/
    cont = create_slider_two(section, NULL, &my_style_pic, "小臂", &my_style_font_DingTalk_JinBuTi_Chinese_16, (int)Arm_Angle_b_Min, (int)Arm_Angle_b_Max, Front_Angle_To_0_Set);
    ...
    /*底座滑杆*/
    cont = create_slider_two(section, NULL, &my_style_pic, "底座", &my_style_font_DingTalk_JinBuTi_Chinese_16, (int)Arm_Angle_c_Min, (int)Arm_Angle_c_Max, Pedestal_Angle_To_0_Set);
...
static lv_obj_t * lv_menu_page_create_encoder_msg(lv_obj_t *parent)
{
    ...
    section = lv_menu_section_create(sub_page);
    /*末端位置*/
    cont = Real_Position_Create(section, "末端位置");
    lv_obj_ARM.Encoder_Msg.label_Real_Position_x = lv_obj_get_child(cont, 4);
    lv_obj_ARM.Encoder_Msg.label_Real_Position_y = lv_obj_get_child(cont, 5);
    lv_obj_ARM.Encoder_Msg.label_Real_Position_z = lv_obj_get_child(cont, 6);
    /*第二块*/
    lv_menu_separator_create(sub_page);
    section = lv_menu_section_create(sub_page);
    /*大臂*/
...
void Arm_Display()
{
    ...
    Angle3f_ Angle = {Rear_Angle, Front_Angle, Pedestal_Angle};
    Point3f_ Point = Arm_Real_Position_Calculate(Angle);
    if(Point.x >= 0) sprintf(data, "+%05.1f", Point.x);
    else             sprintf(data, "%05.1f", Point.x);
    sprintf(data, "%05.1f", Point.x);
    lv_label_set_text(lv_obj_ARM.Encoder_Msg.label_Real_Position_x, data);
    if(Point.y >= 0) sprintf(data, "+%05.1f", Point.y);
    else             sprintf(data, "%05.1f", Point.y);
    lv_label_set_text(lv_obj_ARM.Encoder_Msg.label_Real_Position_y, data);
    if(Point.z >= 0) sprintf(data, "+%05.1f", Point.z);
    else             sprintf(data, "%05.1f", Point.z);
    lv_label_set_text(lv_obj_ARM.Encoder_Msg.label_Real_Position_z, data);
}