...
static void switch_event_feedback(lv_event_t * e)
{
    ...
    {
        Motro_Control_Wheel.feedback_sign = true;
        Motro_Control_Arm.feedback_sign = true;
    }
    else
    {
        Motro_Control_Wheel.feedback_sign = false;
        Motro_Control_Arm.feedback_sign = false;
    }
}
...
static void btn_feedback_rst_event_handler(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) 
    {
        ...
        lv_obj_Motro_Control.Receive_Successful.times = 0;
        lv_obj_Motro_Control.Receive_Failure.times = 0;
        lv_obj_Motro_Control.Verify_Successful.times = 0;
        lv_obj_Motro_Control.Verify_Failure.times = 0;
...
static void slider_event_cb_feedback(lv_event_t * e)
{
    ...
    Motro_Control_Wheel.feedback_error = slider_value;
    Motro_Control_Arm.feedback_error = slider_value;
}
...
void Motro_Control_create(lv_obj_t * parent)
{
    ...
    lv_obj_t * sw_open = lv_obj_get_child(cont, 1);
    lv_obj_set_width(sw_open,70);
    lv_obj_add_event_cb(sw_open, switch_event_feedback, LV_EVENT_VALUE_CHANGED, NULL);
    lv_obj_add_state(sw_open, LV_STATE_CHECKED);
    /*验证错误重新发送次数滑杆*/
    cont = create_slider(section, NULL, NULL, "反馈错误上限", &my_style_font_DingTalk_JinBuTi_Chinese_16, 1, 10, Motro_Control_Wheel.feedback_error);
    ...
    lv_obj_t * btn_rst = lv_obj_get_child(cont, 1);
    lv_obj_set_width(btn_rst,70);
    lv_obj_t* btn_feedback_rst_label = lv_label_create(btn_rst);	
    ...
    lv_obj_add_event_cb(btn_rst, btn_feedback_rst_event_handler,LV_EVENT_ALL,NULL);
    ...
    lv_obj_Motro_Control.Receive_Successful.label = create_text_two(cont, "接收成功", &my_style_font_DingTalk_JinBuTi_Chinese_16, "0", &my_style_font_DingTalk_JinBuTi_English_14);
    lv_obj_Motro_Control.Receive_Failure.label = create_text_two(cont, "接收失败", &my_style_font_DingTalk_JinBuTi_Chinese_16, "0", &my_style_font_DingTalk_JinBuTi_English_14);
    lv_obj_Motro_Control.Verify_Successful.label = create_text_two(cont, "验证成功", &my_style_font_DingTalk_JinBuTi_Chinese_16, "0", &my_style_font_DingTalk_JinBuTi_English_14);
    lv_obj_Motro_Control.Verify_Failure.label = create_text_two(cont, "验证失败", &my_style_font_DingTalk_JinBuTi_Chinese_16, "0", &my_style_font_DingTalk_JinBuTi_English_14);
...
void Motro_Control_Display()
{
    ...
    sprintf(data, "%d", lv_obj_Motro_Control.Receive_Successful.times);
    lv_label_set_text(lv_obj_Motro_Control.Receive_Successful.label, data);
    sprintf(data, "%d", lv_obj_Motro_Control.Receive_Failure.times);
    lv_label_set_text(lv_obj_Motro_Control.Receive_Failure.label, data);
    sprintf(data, "%d", lv_obj_Motro_Control.Verify_Successful.times);
    lv_label_set_text(lv_obj_Motro_Control.Verify_Successful.label, data);
    sprintf(data, "%d", lv_obj_Motro_Control.Verify_Failure.times);
    lv_label_set_text(lv_obj_Motro_Control.Verify_Failure.label, data);
