...
{
	char string[20]={0};
	if(Sensor_Msg.IMU_Data.chart_sign.A)
	{
		lv_chart_set_next_value(lv_obj_IMU.Chart_Acc,lv_obj_IMU.Ax.series,(lv_coord_t)Sensor_Msg.IMU_Data.Ax);
		lv_chart_set_next_value(lv_obj_IMU.Chart_Acc,lv_obj_IMU.Ay.series,(lv_coord_t)Sensor_Msg.IMU_Data.Ay);
		lv_chart_set_next_value(lv_obj_IMU.Chart_Acc,lv_obj_IMU.Az.series,(lv_coord_t)Sensor_Msg.IMU_Data.Az);
		lv_chart_refresh(lv_obj_IMU.Chart_Acc);

		if(Sensor_Msg.IMU_Data.Ax>=0)
			sprintf(string,"Ax:+%.1f",Sensor_Msg.IMU_Data.Ax);
		else
			sprintf(string,"Ax:%.1f",Sensor_Msg.IMU_Data.Ax);
		lv_label_set_text(lv_obj_IMU.Ax.label, string);

		if(Sensor_Msg.IMU_Data.Ay>=0)
			sprintf(string,"Ay:+%.1f",Sensor_Msg.IMU_Data.Ay);
		else
			sprintf(string,"Ay:%.1f",Sensor_Msg.IMU_Data.Ay);
		lv_label_set_text(lv_obj_IMU.Ay.label, string);

		if(Sensor_Msg.IMU_Data.Az>=0)
			sprintf(string,"Az:+%.1f",Sensor_Msg.IMU_Data.Az);
		else
			sprintf(string,"Az:%.1f",Sensor_Msg.IMU_Data.Az);
		lv_label_set_text(lv_obj_IMU.Az.label, string); 
		Sensor_Msg.IMU_Data.chart_sign.A = false;
	}
