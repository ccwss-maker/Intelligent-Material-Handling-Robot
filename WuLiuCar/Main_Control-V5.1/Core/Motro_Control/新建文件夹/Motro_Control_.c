...
void Motro_IDLE_IT(uint8_t rx_len)
{
	...
	{
		if(rx_len == rx_length_set && XOR_Calculate(Receive, rx_length_set-1) == Receive[rx_length_set-1])
		{
			if(Receive_Verify_Sign == true)
			{
				if(memcmp(Receive,Motro_Control.UART.Verify,rx_length_set-1) == 0)
					goto Next;
				else
					goto Retry;
			}
...
void Motor_Open(uint8_t Address)
{
	...
	uint8_t receive_type[2]={Address,0x02};
...
bool Motor_Position_Done(uint8_t length, uint8_t data[])
{
	...
	if(Motro_Control.Angle_Control.Done_Num < Angle_Control_Num && length == 3)
	{
		...
		{
			uint8_t receive_type[3] = {Address, 0x9F, 0x6B};
			receive_type[2] = XOR_Calculate(data, 2);
			uint8_t ID = Address - 1;
			if(memcmp(data, receive_type, 3) == 0)
			{
