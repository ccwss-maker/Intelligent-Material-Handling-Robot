...
static lv_obj_t * lv_menu_page_create_PID(lv_obj_t *parent, const char *const title, lv_obj_PID_KID_ *lv_obj_PID_KID)
{
	...
	lv_obj_PID_KID->btn_save = lv_obj_get_child(cont, 1);
    lv_obj_set_width(lv_obj_PID_KID->btn_save,70);
    lv_obj_add_event_cb(lv_obj_PID_KID->btn_save, btn_event_handler_save,LV_EVENT_ALL,ID_PID);
    lv_obj_add_state(lv_obj_PID_KID->btn_save, LV_STATE_DISABLED);
    lv_obj_PID_KID->btn_save_label = lv_label_create(lv_obj_PID_KID->btn_save);	
    lv_obj_add_style(lv_obj_PID_KID->btn_save_label,&my_style_font_DingTalk_JinBuTi_Chinese_16,LV_STATE_DEFAULT);
    lv_label_set_text(lv_obj_PID_KID->btn_save_label, "执行");
    lv_obj_align(lv_obj_PID_KID->btn_save_label, LV_ALIGN_CENTER, 0, 0);
    /*参数清零*/
    cont = create_btn(section,NULL, NULL, "参数清零", &my_style_font_DingTalk_JinBuTi_Chinese_16);
	lv_obj_PID_KID->btn_clear = lv_obj_get_child(cont, 1);
    lv_obj_set_width(lv_obj_PID_KID->btn_clear,70);
    lv_obj_add_event_cb(lv_obj_PID_KID->btn_clear, btn_event_handler_clear,LV_EVENT_ALL,ID_PID);
    lv_obj_add_state(lv_obj_PID_KID->btn_clear, LV_STATE_DISABLED);
    lv_obj_PID_KID->btn_clear_label = lv_label_create(lv_obj_PID_KID->btn_clear);	
    lv_obj_add_style(lv_obj_PID_KID->btn_clear_label,&my_style_font_DingTalk_JinBuTi_Chinese_16,LV_STATE_DEFAULT);
    lv_label_set_text(lv_obj_PID_KID->btn_clear_label, "执行");
    lv_obj_align(lv_obj_PID_KID->btn_clear_label, LV_ALIGN_CENTER, 0, 0);
    /*微调器*/
    float divisor;
    lv_obj_t* spinbox;
    /*P*/
    cont = create_spinbox(section, NULL, NULL, "P", &my_style_font_DingTalk_JinBuTi_English_14, 5,2,0);
    ...
    /*I*/
    cont = create_spinbox(section, NULL, NULL, "I", &my_style_font_DingTalk_JinBuTi_English_14, 5,2,0);
    ...
    /*D*/
    cont = create_spinbox(section, NULL, NULL, "D", &my_style_font_DingTalk_JinBuTi_English_14, 5,2,0);
...
void PID_Charting()
{
    ...
    if(PID[0].debug_sign == true)
    {
        ...
        lv_chart_set_next_value(lv_obj_PID.speed.Chart.Chart, lv_obj_PID.speed.Chart.ref.series, 10*message_imu.Wz);
        ...
        if(message_imu.Wz>=0)
            sprintf(string,"ref : +%.1f",message_imu.Wz);
        else
            sprintf(string,"ref : %.1f",message_imu.Wz);
        ...
        PID[0].debug_set = slider_val;
    }
    /*位置环信息*/
    else if(PID[1].debug_sign == true)
    {
        slider_val = (lv_coord_t)lv_slider_get_value(lv_obj_PID.position.slider_value_set)/10.0;
        lv_chart_set_next_value(lv_obj_PID.position.Chart.Chart, lv_obj_PID.position.Chart.ref.series, 10*message_imu.Yaw);
        ...
        if(message_imu.Yaw>=0)
            sprintf(string,"ref : +%.1f",message_imu.Yaw);
        else
            sprintf(string,"ref : %.1f",message_imu.Yaw);
...
void PID_Debug_Set_BLE(uint8_t ID_PID)
{
    ...
    int slider_value;
    if(ble_data.L_X>-50&&ble_data.L_X<50)	ble_data.L_X=0;
    ...
    {
        slider_value = ble_data.L_X*(((lv_bar_t *)slider)->max_value-((lv_bar_t *)slider)->min_value)/3500;
    }
